...
/**
 * PointEvents inherits from Event. 
 * Objects of type PointEvent don't have a next and previous reference, i.e. it is set to null.
 * They are just Points. We can delete them, edit it, i.e. update its attributes or transform 
 * it to an object of type RepeatingEvent. furthermore we cannot generate successor events of a point event, 
 * since a point event has no next reference. 
 * @author team1
 *
 */
public class PointEvent extends Event{
	 ...
	 * @param end end  date/time for this event
	...
	/**
	 * This constructor transform an InvervalEvent into a PointEvent.
	 * preserve id of given RepeatingEvent by calling forceSetId() and set originId
	 * @param intervalEvent from which we take its values 
	 */
	...
	/**
	 * This constructor transform an Repeating Event into a PointEvent.
	 * preserve id of given RepeatingEvent by calling forceSetId() and set originId
	 * @param repeatingEvent from which we take its values 
	 */
	...
	/**
	 * sets the next event for this event, but this is a point event 
	 * and since a point event has no next event, don't do anything here.
	 */
	// TODO maybe throw later an exception to notify programmers they did something against the model
	public void setNext(Event event){}
	
	/**
	 * sets the previous event for this event, but this is a point event 
	 * and since a point event has no previous event, don't do anything here.
	 * @param event for a point event we don't set the given event ad next
	 */
	@Override
	public void setPrevious(Event event) {}
	
	/**
	 * generates the following events for this event but since this event is a point event and point events don't have a successor, 
	 * this method does nothing but return null
	 */
	@Override
	...
	/**
	 * remove this event, i.e. for a PointEvent: 
	 * just remove this event from its calendar's head list.
	 */
	@Override
	public void remove() {
	...
	public void edit(String name, DateTime start, DateTime end,
		...
		// if interval not 0, here we have create a new object of type RepeatingEvent
