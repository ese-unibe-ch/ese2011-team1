...
public class Calendar {
	...
	/*
	 * public Event getEventById(long id){ for(Event event : this.eventHeads){
	 * return event.findEventById(id); } return null; }
	 */
	...
	// or added a new event an declared him as an IntervalEvent or
	// RepeatingEvent
	// or modified an existing event and changed him to an IntervalEvent or
	// RepeatingEvent
	 ...
	 * head and its tail from this calendar.
	 * 
	 * @param member
	...
	public void removeSerieOfRepeatingEvents(Event member) {
		...
		/*
		 * long baseId = member.getBaseId(); Event victimHead =
		 * getHeadById(baseId); this.getHeadList().remove(victimHead);
		 */
	}
	...
	public void cancelRepeatingEventRepetitionFromDate(Event cancelFromThis) {
		...
		previous.setNext(intervalCursor);
		intervalCursor.setPrevious(previous);
		Event newEnd = new IntervalEvent(victimHead.getStart(),
	...
	public boolean hasEventOnDate(LocalDate date, User requester) {
		for (Event event : this.eventHeads) {
			Event cursor = event;
			if (cursor.happensOn(date))
				if (requester == owner || cursor.getVisibility() != Visibility.PRIVATE)
					return true;
	...
	/**
	 * for debugging print head and its tail, the member itself can be any
	 * element of such tail-head structure.
	 * 
	 * @param member
	 *            a member of a head and it's tail.
	 */
	public void PrintHeadAndHisTail(Event member) {
		long baseId = member.getBaseId();
		LinkedList<Event> events = this.getSameBaseIdEvents(baseId);
		for (Event event : events)
			System.out.println(event.getParsedStartDate() + " baseid: "
					+ event.getBaseId() + " id " + event.getId());
	}

	/**
	 * for debugging print for all heads in getHeadList the head and its tail,
	 */
	public void PrintAllHeadTails() {
		for (Event event : this.getHeadList()) {
			System.out.println();
			System.out.println("head: " + event.getParsedStartDate() + " id:"
					+ event.getId());
	...
	/**
	 * print current, next and previous events starting from a head till its
	 * tail. for debugging purposes: see if this event has correct next and
	 * previous reference remove this method later, just a private helper.
	 * 
	 * @param head
	 *            head of a series of events
	 */
	private void newNextEventsPrinter(Event head) {
		Event event = head;
		String ee = null;
		while (event.hasNext()) {
			if (event.getPreviousReference() != null)
				ee = event.getPreviousReference().getParsedStartDate();
			System.out.println("current: " + event.getParsedStartDate()
					+ " nextR:" + event.getNextReference().getParsedStartDate()
					+ " prevR:" + ee);
			event = event.getNextReference();
		}
		ee = null;
		if (event.getNextReference() != null)
			ee = event.getNextReference().getParsedStartDate();
		System.out.println("current: " + event.getParsedStartDate() + " nextR:"
				+ ee + "              prevR:"
				+ event.getPreviousReference().getParsedStartDate());
	}
}
